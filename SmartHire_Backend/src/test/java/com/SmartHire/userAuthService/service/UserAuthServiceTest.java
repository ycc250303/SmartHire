 ...
 *
 * <p>测试目标：防止用户注册、登录、Token刷新等核心业务逻辑回归
 *
 * <p>测试原则：
 *
 * <ul>
 *   <li>使用 Mockito mock 外部依赖（验证码服务、数据库、JWT工具、Redis）
 *   <li>保留真实业务逻辑测试（密码加密、Token生成、用户信息查询）
...
class UserAuthServiceTest {
  ...
  @Mock private JwtTokenExtractor tokenExtractor;
  ...
  void testRegister_Success() {
    ...
    doNothing()
        ...
        .verifyCodeWithoutDelete(eq(VALID_EMAIL), eq(VALID_CODE));
  ...
  void testRegister_EmailExists() {
    ...
    doNothing()
        .when(verificationCodeService)
        .verifyCodeWithoutDelete(eq("existing@example.com"), eq(VALID_CODE));
  ...
  void testLogin_Success() {
    ...
    ArgumentCaptor<Map<String, Object>> accessTokenClaimsCaptor =
        ArgumentCaptor.forClass(Map.class);
    ArgumentCaptor<Map<String, Object>> refreshTokenClaimsCaptor =
        ArgumentCaptor.forClass(Map.class);
    ...
    assertEquals(
        VALID_USERNAME, accessTokenClaims.get("username"), "AccessToken claims 应该包含正确的用户名");
    ...
    assertEquals(
        VALID_USERNAME, refreshTokenClaims.get("username"), "RefreshToken claims 应该包含正确的用户名");
  ...
  void testRefreshToken_Success() {
    ...
    com.auth0.jwt.interfaces.DecodedJWT newAccessDecoded =
        mock(com.auth0.jwt.interfaces.DecodedJWT.class);
  ...
  void testLogout_Success() {
    ...
    assertTrue(
        expireTimeCaptor.getAllValues().stream().allMatch(time -> time > 0), "黑名单过期时间应该大于 0");
