 ...
 *
 * <p>测试目标：防止求职者信息注册、更新、查询等核心业务逻辑回归
 *
 * <p>测试原则：
 *
 * <ul>
 *   <li>使用 Mockito mock 外部依赖（UserContext、数据库Mapper）
...
class JobSeekerServiceTest {
  ...
  @Spy @InjectMocks private JobSeekerServiceImpl jobSeekerService;
  ...
  private void mockLambdaQueryResult(JobSeeker result) {
    ...
    LambdaQueryChainWrapper<JobSeeker> queryWrapper =
        new LambdaQueryChainWrapper<>(jobSeekerMapper, JobSeeker.class);

    // Mock lambdaQuery() 返回真正的 LambdaQueryChainWrapper 实例
    doReturn(queryWrapper).when(jobSeekerService).lambdaQuery();
  ...
  /**
   * Mock getJobSeekerEntity() 方法，返回指定的 JobSeeker 对象 注意：这是私有方法，需要通过反射来 mock
   *
   * @param result 查询结果（null 表示不存在）
   */
  ...
  void testRegisterSeeker_Success() throws ParseException {
    ...
    assertEquals(
        DATE_FORMAT.parse(TEST_BIRTH_DATE_STR), insertedJobSeeker.getBirthDate(), "出生日期应该正确设置");
  ...
  void testRegisterSeeker_UserIdNotExist() throws ParseException {
    ...
    assertEquals("用户ID不存在", exception.getMessage(), "异常消息应该正确");
  ...
  void testUpdateSeekerInfo_Success() throws ParseException {
    ...
    assertEquals(DATE_FORMAT.parse("1999-05-15"), updatedJobSeeker.getBirthDate(), "出生日期应该正确更新");
  ...
  void testUpdateSeekerInfo_SeekerNotExist() throws ParseException {
    ...
    assertEquals("求职者信息不存在", exception.getMessage(), "异常消息应该正确");
  ...
  void testGetSeekerInfo_Success() throws ParseException {
    ...
    assertEquals(DATE_FORMAT.parse(TEST_BIRTH_DATE_STR), result.getBirthDate(), "出生日期应该正确映射");
