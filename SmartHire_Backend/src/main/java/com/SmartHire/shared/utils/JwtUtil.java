...
public class JwtUtil {
    ...
    /**
     * 验证令牌（基础方法）
     * 验证 token 的有效性（签名、过期时间等），返回完整的 DecodedJWT
     * 适用于需要检查 token 类型、过期时间等完整信息的场景
     *
    ...
    /**
     * 解析令牌并提取 claims
     * 内部调用 verifyToken 进行验证，然后提取 claims 信息
     * 适用于只需要用户信息（claims）的场景
     *
     * @param token JWT token 字符串
     ...
     * 辅助方法，用于从已验证的 DecodedJWT 中提取 claims
     *
