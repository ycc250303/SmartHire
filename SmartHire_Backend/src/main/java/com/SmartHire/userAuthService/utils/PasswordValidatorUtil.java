...
public class PasswordValidatorUtil implements ConstraintValidator<ValidPasswordUtil, String> {
    ...
    public boolean isValid(String password, ConstraintValidatorContext context) {
        ...
        if (password.length() < 6) {
            ...
            context.buildConstraintViolationWithTemplate("密码长度至少6位")
        ...
        if (!password.matches("^[a-zA-Z0-9]+$")) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("密码不能包含特殊字符，只能包含字母和数字")
                    .addConstraintViolation();
            return false;
        }
        ...
        if (typeCount < 2) {
            context.disableDefaultConstraintViolation();
            context.buildConstraintViolationWithTemplate("密码至少包含数字、小写字母、大写字母的两项")
                    .addConstraintViolation();
            return false;
        }
