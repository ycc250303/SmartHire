...
public class EducationExperienceServiceImpl extends ServiceImpl<EducationExperienceMapper, EducationExperience>
    ...
    public void updateEducationExperience(Long id, EducationExperienceDTO request) {
        ...
        // 查询教育经历是否存在且属于当前用户
        EducationExperience existingExperience = educationExperienceMapper.selectById(id);
        if (existingExperience == null) {
            throw new BusinessException(ErrorCode.EDUCATION_EXPERIENCE_NOT_EXIST);
        }
        if (!existingExperience.getJobSeekerId().equals(jobSeekerId)) {
            throw new BusinessException(ErrorCode.EDUCATION_EXPERIENCE_NOT_BELONG_TO_USER);
        }
