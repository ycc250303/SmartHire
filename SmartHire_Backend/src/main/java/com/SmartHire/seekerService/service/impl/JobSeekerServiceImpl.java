...
public class JobSeekerServiceImpl extends ServiceImpl<JobSeekerMapper, JobSeeker> implements JobSeekerService {
     ...
     * 
     * 效率分析：
     * 1. 直接使用Mapper批量删除（当前实现）：
     * - 每条表：1条SQL批量删除所有记录
     * - Resume特殊处理：1次查询获取fileUrl + 1次批量删除SQL + N次OSS删除
     * - 总操作数：查询次数 + 删除SQL次数 + OSS删除次数
     * 
     * 2. 复用Service接口（不推荐）：
     * - 每条表：1次查询获取ID列表 + N次deleteById（N条SQL）+ N次权限验证
     * - Resume：1次查询获取ID列表 + N次deleteById + N次权限验证 + N次OSS删除
     * - 总操作数：查询次数 + N*删除SQL次数 + N*权限验证 + N*OSS删除
     * 
     * 结论：直接使用Mapper批量删除效率更高，因为：
     * - 批量删除：1条SQL vs N条SQL（N是记录数）
     * - 不需要权限验证（删除求职者时已经是自己的数据）
     * - 代码更简洁
     *
    ...
    private void deleteResumeModuleData(Long jobSeekerId) {
        ...
        jobSeekerExpectationMapper.delete(
                new LambdaQueryWrapper<com.SmartHire.seekerService.model.JobSeekerExpectation>()
                        .eq(com.SmartHire.seekerService.model.JobSeekerExpectation::getJobSeekerId, jobSeekerId));
        ...
        educationExperienceMapper.delete(
                new LambdaQueryWrapper<com.SmartHire.seekerService.model.EducationExperience>()
                        .eq(com.SmartHire.seekerService.model.EducationExperience::getJobSeekerId, jobSeekerId));
    ...
    public void updateEducationAndGraduationYear(Long jobSeekerId) {
        ...
        if (jobSeeker != null) {
            ...
            jobSeeker.setUpdatedAt(new Date());
            jobSeekerMapper.updateById(jobSeeker);
            log.info("自动更新求职者最高学历和毕业年份，求职者ID：{}，最高学历：{}，毕业年份：{}",
                    jobSeekerId, maxEducation, latestGraduationYear);
        }
    }
}
