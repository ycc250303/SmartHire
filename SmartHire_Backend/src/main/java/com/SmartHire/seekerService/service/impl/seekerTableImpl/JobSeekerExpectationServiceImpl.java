...
public class JobSeekerExpectationServiceImpl
  ...
  public void addJobSeekerExpectation(@Valid JobSeekerExpectationDTO request) {
    ...
    // 检查是否已达到上限（最多5个）
    long currentCount = lambdaQuery().eq(JobSeekerExpectation::getJobSeekerId, jobSeekerId).count();
   ...
   * @param source
   */
  private void populateExpectationForCreate(
      JobSeekerExpectation target, JobSeekerExpectationDTO source) {
    target.setExpectedPosition(source.getExpectedPosition());
    // 期望行业为非必填项，默认为"不限行业"
    target.setExpectedIndustry(
        StringUtils.hasText(source.getExpectedIndustry()) ? source.getExpectedIndustry() : "不限行业");
   ...
   * @return
   */
  private boolean applyExpectationUpdates(
      JobSeekerExpectation target, JobSeekerExpectationDTO source) {
    ...
    if (source.getExpectedIndustry() != null) {
      // 如果传入空字符串，设置为"不限行业"
      target.setExpectedIndustry(
          StringUtils.hasText(source.getExpectedIndustry())
