...
public class BanRecordServiceImpl extends ServiceImpl<BanRecordMapper, BanRecord> implements BanRecordService {
    ...
    public Map<String, Object> getBanStatistics() {
        ...
        // 生效中
        int activeCount = Math.toIntExact(this.lambdaQuery()
                .eq(BanRecord::getBanStatus, "active")
                .count());
        statistics.put("active", activeCount);

        // 已过期
        int expiredCount = Math.toIntExact(this.lambdaQuery()
                .eq(BanRecord::getBanStatus, "expired")
                .count());
        statistics.put("expired", expiredCount);

        // 已解除
        int liftedCount = Math.toIntExact(this.lambdaQuery()
                .eq(BanRecord::getBanStatus, "lifted")
                .count());
