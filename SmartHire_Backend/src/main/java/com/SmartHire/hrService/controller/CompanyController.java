...
public class CompanyController {
     ...
     * 创建公司
     *
     * @param createDTO 公司信息
     * @return 操作结果
     */
    @PostMapping("/company")
    @Operation(summary = "创建公司", description = "创建新公司，并等待管理员审核")
    public Result<?> createCompany(@Valid @RequestBody CompanyDTO createDTO) {
        Long companyId = companyAdminService.createCompany(createDTO);
        return Result.success("公司创建成功", companyId);
    }
    ...
    @Operation(summary = "获取公司详细信息", description = "获取公司详细信息")
    public Result<?> getCompanyById(@PathVariable Long companyId) {
        CompanyDTO company = companyAdminService.getCompanyById(companyId);
        return Result.success("获取公司信息成功", company);
    }
    ...
    public Result<?> getCompanies(
        ...
        List<CompanyNameDTO> result = companyAdminService.getCompanies(current, size, keyword);
     ...
     * @return 操作结果
     */
    @PostMapping("/{companyId}/info")
    @Operation(summary = "修改公司信息", description = "公司管理员修改本公司信息")
    public Result<?> updateCompanyInfo(
            @PathVariable Long companyId, @Valid @RequestBody CompanyDTO companyDTO) {
        companyAdminService.updateCompanyInfo(companyId, companyDTO);
