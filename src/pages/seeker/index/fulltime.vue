...
import { getFullTimeJobRecommendationsWithSupplement, getLatestFullTimeJobs, type FullTimeJobItem } from '@/services/api/recommendations';
...
const loadJobs = async () => {
  ...
  error.value = null;
  try {
    let response: { jobs: FullTimeJobItem[] };

    switch (props.filter) {
      case 'latest':
        response = await getLatestFullTimeJobs({ page: 1, size: 20 });
        break;
      default:
        response = await getFullTimeJobRecommendationsWithSupplement();
        break;
    }
