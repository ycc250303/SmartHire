...
function navigateToUpload(filePath: string, fileName: string) {
  ...
  uni.navigateTo({
    url: `/pages/profile/resume/edit/upload-resume?filePath=${encodeURIComponent(filePath)}&defaultName=${encodeURIComponent(defaultName)}`,
  });
}

function handleEditResume(resume: Resume) {
  uni.navigateTo({
    url: `/pages/profile/resume/edit/edit-resume?id=${resume.id}`,
  });
}
