<template>
  <view class="favorite-job-card" @click="handleCardClick">
    ...
    <view class="job-meta">
      ...
      <view class="tag tag-salary" v-if="job.salaryMin && job.salaryMax">{{ formatSalary(job.salaryMin, job.salaryMax) }}</view>
    </view>
    
    <view class="job-tags">
      <view class="tag tag-degree" v-if="job.educationRequired !== undefined && job.educationRequired !== null">{{ getDegreeText(job.educationRequired) }}</view>
    </view>
  </view>
</template>
...
import type { FavoriteJob } from '@/services/api/seeker';
