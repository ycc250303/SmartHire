...
/**
 * Set both access token and refresh token with expiry time (preferred)
 */
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        if (apiResponse.code === SUCCESS_CODE && apiResponse.data) {
          setTokens(apiResponse.data.accessToken, apiResponse.data.refreshToken, apiResponse.data.expiresIn);
        } else {
